...
int PipeServer::createSecurityAttributes(SECURITY_ATTRIBUTES* sa)
{
	...
	// Создать известный SID для группы «Все»
	...
	// ACE позволит каждому иметь полный контроль над объектом
	...
	// Создаем новый ACL, который содержит новые ACE.
	...
	// Добавить ACL к дескриптору безопасности.
	...
	// Инициализация структуры атрибутов безопасности.
...
void PipeServer::createNamedPipe()
{
	...
	{
		std::string error("[-] getSecurityDescriptor failed : " + std::to_string(GetLastError()));
		throw error;
	}
	...
	{
		std::string error("[-] CreateNamedPipe failed : " + std::to_string(GetLastError()));
		throw error;
	}
}
...
void PipeServer::waitForClient()
{
	...
	while (true)
	{
		...
		// Ожидание подключения клиента; если это удастся,
...
int PipeServer::receiveMessage(std::string& message)
{
	...
	if (!fSuccess)
	{
		std::string error("[-] ReadFile failed : " + std::to_string(GetLastError()));
		throw error;
	}
...
void PipeServer::sendMessage(const std::string& message)
{
	...
	{
		std::string error("[-] WriteFile failed : " + std::to_string(GetLastError()));
		throw error;
	}
}
